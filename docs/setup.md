...
- Amazon Bedrock is the default live model provider. Set `MODEL_MODE=live`, keep `MODEL_PROVIDER=bedrock`, `AWS_REGION=us-west-2`, and `BEDROCK_MODEL_ID=us.amazon.nova-2-lite-v1:0`, and make AWS credentials available through the standard SDK credential chain. Locally that can be a reviewed shared AWS profile or server-only environment credentials; on AWS, prefer an attached least-privileged IAM role. The principal needs only the Bedrock invocation permissions used by the selected inference profile and its destination foundation models. Do not put AWS credentials in `.env.example` or any `VITE_*` variable. Restart, open the CopilotKit conversation, and verify a streamed response and a persisted proposal. Configuration or runtime discovery is not proof of model access.
