...
Historical interface snapshot reviewed **2026-09-12**; the default model configuration was updated **2026-09-14**. Documentation and schema retrieval succeeded. This proves the documented interface, not access to a customer's account. The initial fixture build used no model or task-provider credentials and made no live task writes. OpenRouter was the default selectable live model vendor in that dated snapshot. The current configuration defaults to Amazon Bedrock and Amazon Nova 2 Lite; dated OpenRouter results below remain historical evidence and are not relabeled as Bedrock results. Current credential-backed results belong in the [test report](tests.md), separately from fixture and mocked-transport checks.
...
The server selects exactly one vendor with `MODEL_PROVIDER`. The default is `bedrock`, using `AWS_REGION=us-west-2` and the US geo inference profile `BEDROCK_MODEL_ID=us.amazon.nova-2-lite-v1:0`. Bedrock authentication comes from the AWS SDK's standard credential chain: prefer an attached least-privileged IAM role on AWS, or a reviewed shared profile or server-only environment credentials for local development. OpenRouter remains available with `MODEL_PROVIDER=openrouter`, `OPENROUTER_API_KEY`, and `OPENROUTER_MODEL` (default `openai/gpt-5.6-luna`); direct OpenAI remains available with `MODEL_PROVIDER=openai`, `OPENAI_API_KEY`, and `OPENAI_MODEL` (default `gpt-5-mini`). `MODEL_MODE=live` must be explicit. Missing authentication and vendor errors do not try another vendor or select fixture mode. Task `PROVIDER_MODE` is independent. [Amazon Nova 2 Lite model card](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-amazon-nova-2-lite.html), [AWS SDK credential chain](https://docs.aws.amazon.com/sdk-for-javascript/v3/developer-guide/setting-credentials-node.html)

For Nova 2 Lite, the AWS principal needs `bedrock:InvokeModel` and, for streaming, `bedrock:InvokeModelWithResponseStream`, scoped to the configured inference profile and its destination foundation-model ARNs. Keep deployment and administrator permissions out of the application role. Successful credential discovery or `sts:GetCallerIdentity` does not prove that the selected Bedrock model can be invoked; verify a bounded model request separately.
