...
function ExecutionStartForm({capabilities,loading,paired,error,onRefresh,onStarted,onCancel,onPlanningSetup}:{capabilities:ExecutionCapabilities|null;loading:boolean;paired:boolean;error:string;onRefresh:()=>Promise<void>;onStarted:Props['onStarted'];onCancel?:()=>void;onPlanningSetup?:()=>void}){
  ...
  const [template,setTemplate]=useState<'generic'|'adaptive_launch'>('generic');
  const [sources,setSources]=useState<AdaptiveSourceInput[]>([{id:'source-1',title:'',content:'',provenance:'pasted'}]);
  ...
  return <section className="execution-start" aria-label="Start mission execution">
    ...
    <p className="execution-start-intro">State the outcome and add your source material. MissionDeck breaks down the work, saves tasks in Ambiguous, and starts supported work for the selected agent when each task is ready.</p>
    ...
    <div className="adaptive-template-options" role="group" aria-label="Mission workflow"><label className="adaptive-template-option"><input type="radio" name="execution-template" value="generic" checked={template==='generic'} onChange={()=>setTemplate('generic')} disabled={disabled}/><span><strong>Draft and review</strong><small>Draft from your context, collect human feedback, and prepare a final document.</small></span></label><label className="adaptive-template-option"><input type="radio" name="execution-template" value="adaptive_launch" checked={template==='adaptive_launch'} onChange={()=>setTemplate('adaptive_launch')} disabled={disabled}/><span><strong>Adaptive launch review</strong><small>Compare named sources, decide how to handle conflicts, and update the launch pack when sources change.</small></span></label></div>
    <Button type="button" variant="secondary" disabled={disabled} onClick={()=>{
      setMaxTasks(3);setMaxAgentRuns(template==='adaptive_launch'?8:4);
      if(template==='adaptive_launch'){
        setGoal("Prepare Harbor's launch pack and resolve the calendar conflict.");
        setSources(ADAPTIVE_SAMPLE_SOURCES.map(source=>({...source})));
      }else{
        setGoal('Prepare a launch brief from these demo sources, ask me to review the positioning, then create the final brief.');
        setContext('Fictional demo sources — these are sample facts, not real business claims.\n\nProduct: Harbor, a shared workspace for small project teams.\nAudience: teams coordinating human and agent work.\nCore features: mission planning, named task ownership, human review, and a shared document library.\nPositioning: make it clear who is doing what and where the latest work lives.\nConstraints: no invented customer quotes, revenue, performance metrics, pricing, or launch dates.\nDeliverable: a concise launch brief with audience, positioning, features, and a launch checklist.\nReview: ask the human reviewer to confirm the positioning before writing the final brief.');
      }
      if(!humanId&&capabilities?.humans[0])setHumanId(capabilities.humans[0].id);if(!agentId&&capabilities?.agents[0])setAgentId(capabilities.agents[0].id);
    }}>{template==='adaptive_launch'?'Try the adaptive launch sample':'Try a sample mission'}</Button>
    ...
    <form onSubmit={event=>void start(event)}>
      ...
      <p className="execution-authorization">Starting authorizes this mission to create and assign tasks, run {template==='adaptive_launch'?'the Strands agents':'the server drafting worker'} within these limits, and save the mission context, {template==='adaptive_launch'?'source revisions, analysis, decisions, and launch pack':'drafts, reviews, and deliverables'} in {capabilities?.mode==='fixture'?'the fixture workspace':'your connected Ambiguous workspace'}. {template==='adaptive_launch'?'Your launch decision releases the final drafting work.':'Human review pauses dependent work until a review is saved.'}</p>
      ...
      <div className="execution-start-actions"><Button type="submit" busy={busy} disabled={!paired||loading||!executionEnabled||!selectedHuman||!selectedAgent||goal.trim().length<5||!adaptiveReady}>{uncertain?'Recover mission':busy?'Starting your mission…':'Start mission'}<ArrowRight size={15}/></Button>{onCancel&&<Button type="button" variant="ghost" disabled={busy||uncertain} onClick={onCancel}>Cancel</Button>}{onPlanningSetup&&<Button type="button" variant="ghost" disabled={busy||uncertain} onClick={onPlanningSetup}>Use planning-only setup</Button>}</div>
    </form>
  </section>;
}
...
function Artifact({artifact,taskTitle}:{artifact:ExecutionArtifact;taskTitle?:string}){
  ...
  return <details className="execution-artifact"><summary><span className="execution-artifact-title"><FileText size={15}/>{artifact.title}</span><Badge tone={artifact.mode==='fixture'?'amber':'green'}>{artifact.mode==='fixture'?'Fixture saved':'Verified in Ambiguous'}</Badge></summary><div className="execution-artifact-meta">{artifact.sourceRevision!==undefined&&<span>Source revision: {artifact.sourceRevision}</span>}{taskTitle&&<span>Task: {taskTitle}</span>}<span>Document ID: <code>{artifact.id}</code></span><span>Verified {dateTime(artifact.verifiedAt)}</span>{link&&<a href={link} target="_blank" rel="noreferrer">Open in Ambiguous <ExternalLink size={11}/></a>}</div><pre className="execution-artifact-content">{artifact.content}</pre></details>;
}
