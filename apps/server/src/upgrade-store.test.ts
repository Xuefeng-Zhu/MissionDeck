...
it('applies additive versions once and rolls back a failed version',async()=>{
 db=await openDatabase({memory:true});expect((await db.query('SELECT version FROM schema_migrations ORDER BY version')).rows).toEqual([{version:1},{version:2},{version:3},{version:4}]);
 await applyMigrations(db);
 const directory=await mkdtemp(join(tmpdir(),'mission-migrations-'));
 try{await writeFile(join(directory,'005_test.sql'),'CREATE TABLE rollback_probe(id text); INVALID SQL;');await expect(applyMigrations(db,pathToFileURL(directory+'/'))).rejects.toThrow();expect((await db.query("SELECT tablename FROM pg_tables WHERE tablename='rollback_probe'")).rows).toEqual([]);expect((await db.query('SELECT version FROM schema_migrations WHERE version=5')).rows).toEqual([]);}finally{await rm(directory,{recursive:true,force:true});}
});
