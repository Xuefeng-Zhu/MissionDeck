...
The copied environment is intentionally all-fixture: it can load the local UI and run deterministic tests without an external call, but it does not enable the adaptive Harbor workflow. To run that workflow interactively, keep `PROVIDER_MODE=fixture`, set `MODEL_MODE=live`, and use the default Amazon Bedrock configuration with AWS credentials available through the standard credential chain. The default model is Amazon Nova 2 Lite through the US geo inference profile `us.amazon.nova-2-lite-v1:0` in `us-west-2`. OpenRouter and direct OpenAI remain explicit opt-in alternatives. Then start MissionDeck:
...
Adaptive launch review requires `MODEL_MODE=live` in normal use. The default is Amazon Bedrock (`MODEL_PROVIDER=bedrock`, `AWS_REGION=us-west-2`, `BEDROCK_MODEL_ID=us.amazon.nova-2-lite-v1:0`) authenticated through the standard AWS credential chain. Prefer an attached least-privileged IAM role on AWS. OpenRouter (`MODEL_PROVIDER=openrouter`) and direct OpenAI (`MODEL_PROVIDER=openai`) remain explicit opt-in alternatives. Credentials remain server-side. Never commit `.env` or paste credentials into issues, screenshots, or demo materials. See [setup](docs/setup.md) for the complete configuration and safety gates.
...
After the reviewed branch is merged to the public default branch, import the repository as a Render Blueprint. The free Blueprint path selects Amazon Nova 2 Lite on Bedrock and prompts for a least-privileged `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`; those values enter the standard AWS environment credential chain and must remain Render secrets. Render plans that support OIDC should use `AWS_ROLE_ARN` and Render's web-identity token instead of long-lived keys; an AWS-hosted deployment with an attached IAM role is also preferable for long-lived operation. Grant only the Bedrock invocation permissions required for the configured inference profile, and set an AWS budget as the outer kill switch. Do not place credentials in Git, Blueprint YAML values, screenshots, or logs.
